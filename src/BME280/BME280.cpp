...
BME280::BME280(i2c_inst_t* i2c, uint8_t address, uint sda_pin, uint scl_pin)
...
bool BME280::readSensor(float& temperature, float& pressure, float& humidity) {
...
void BME280::readRegisters(uint8_t reg, uint8_t* buf, size_t len) {
    i2c_write_blocking(_i2c, _address, &reg, 1, true);
